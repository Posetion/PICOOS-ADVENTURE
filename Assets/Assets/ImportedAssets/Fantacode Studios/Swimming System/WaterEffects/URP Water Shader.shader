Shader "Universal Render Pipeline/URP Water Shader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Calculate wave normal (no ripples)
            ...
            Varyings vert(Attributes input)
            {
                ...
                // Apply wave displacement (ripples removed)
                ...
                float3 waveNormal = GetWaveNormal(positionWS);
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                float2 animatedUV1 = input.uv + timeScale * float2(0.02, 0.03);
                float2 animatedUV2 = input.uv + timeScale * float2(-0.015, 0.025);

                float3 normal1 = UnpackNormal(SAMPLE_TEXTURE2D(_NormalMap, sampler_NormalMap, animatedUV1 * 2.0));
                float3 normal2 = UnpackNormal(SAMPLE_TEXTURE2D(_NormalMap, sampler_NormalMap, animatedUV2 * 1.5));
                float3 tangentNormal = normalize(normal1 + normal2);

                // Transform to world space
                float3x3 TBN = float3x3(normalize(input.tangentWS), normalize(input.binormalWS), normalize(input.normalWS));
                float3 normalWS = normalize(mul(tangentNormal, TBN));

                // Combine with procedural waves
                float3 waveNormal = GetWaveNormal(positionWS);
                normalWS = normalize(lerp(normalWS, waveNormal, 0.3));

                // Normal flip check
                float normalDot = dot(normalWS, viewDirWS);
                if (normalDot < 0)
                {
                    normalWS = -normalWS;
                }
                ...
                float3 diffuse = mainLight.color.rgb * NdotL * 0.6;
                float specPower = exp2(10 * _Shininess + 1);
                float3 specular = mainLight.color.rgb * pow(NdotH, specPower) * _Shininess * 1.5;

                // Fresnel effect
                float fresnel = pow(1.0 - saturate(abs(normalDot)), _FresnelPower);

                // Refraction using opaque texture
                float2 refractionUV = screenUV + normalWS.xz * _Refraction * saturate(depthDifference * 0.1);
                refractionUV = saturate(refractionUV);
                float3 refractedColor = SampleSceneColor(refractionUV);
                ...
                // Reflection
                float2 reflectionUV = screenUV + normalWS.xz * _ReflectionDistortion;
                reflectionUV = saturate(reflectionUV);
                float3 reflection = SAMPLE_TEXTURE2D(_ReflectionTex, sampler_ReflectionTex, reflectionUV).rgb;
